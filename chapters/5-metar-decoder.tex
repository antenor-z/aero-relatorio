...
polegadas de mercúrio (mmHg), mas no Brasil esta não é usada no METAR. O regex é \texttt{(Q[0-9]{4})}
...
Existe também o tipo OVC (overcast) que se refere a totalmente encoberto. O regex usado é \texttt{([A-Z]{3})([0-9]{3})(TCU)?}.
...
A maior expressão regex deste decoder é 
