...
\begin{verbatim}
  \texttt{Vento \$1° com \$2 nós e rajadas de até \$3 nós}, 
\end{verbatim}
...
Considerando que todos as expressões usadas são simples, isto é não leva a backtracking, 
a complexidade temporal para fazer uma execução da função \texttt{re.findall()} do Python
é
...
\begin {verbatim}
m := número de caracteres da expressão regex
n := número de caracteres da string a ser analisada
\end {verbatim}
...
\begin {verbatim}
m := número de caracteres da expressão regex
n := número de caracteres da string do METAR a ser analisado
p := número de items do METAR
q := número de expressões regex no programa
\end {verbatim}
